...
const ChannelGrid: React.FC<ChannelGridProps> = ({ onSelectChannel, searchQuery }) => {
  ...
  const filteredChannels = channels.filter(channel => {
    ...
    if (activeFilter === 'SPORTS') return name.includes('sport') || name.includes('bein') || name.includes('dazn');
    if (activeFilter === 'ARABIC') return name.includes('ar:') || name.includes('quran') || /[\u0600-\u06FF]/.test(name);
    return true;
  });
    ...
    <div className="w-full max-w-7xl mx-auto space-y-12">
      ...
      <div className="flex flex-col lg:flex-row gap-8 justify-between items-start lg:items-center bg-white/5 p-10 rounded-[3rem] border border-white/10 backdrop-blur-2xl relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-4">
             ...
             <h2 className="text-4xl font-black text-white tracking-tighter uppercase">TV <span className="text-indigo-400">Channels</span></h2>
          </div>
          <p className="text-slate-400 font-medium max-w-md">Access high-quality streams from global sports networks instantly.</p>
        </div>
        ...
        <AnimatePresence>
            {filteredChannels.map((channel, idx) => (
                <motion.button
                    ...
                    className="group relative p-8 bg-white/[0.03] hover:bg-white/[0.08] border border-white/5 hover:border-indigo-500/40 rounded-[2.5rem] transition-all overflow-hidden flex flex-col items-center text-center shadow-2xl"
                >
                    {/* Status Glow */}
                    <div className="absolute top-4 right-6 flex items-center gap-2">
                      <span className="text-[8px] font-black text-emerald-500 tracking-widest uppercase opacity-0 group-hover:opacity-100 transition-opacity">Online</span>
                      <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.8)] animate-pulse" />
                    </div>

                    <div className="w-20 h-20 rounded-[2rem] bg-black/40 border border-white/10 flex items-center justify-center mb-6 group-hover:bg-gradient-to-br group-hover:from-indigo-500 group-hover:to-blue-600 transition-all duration-500 shadow-inner group-hover:rotate-6">
                         {channel.logo ? (
                           <img src={channel.logo} alt="" className="w-12 h-12 object-contain" referrerPolicy="no-referrer" />
                         ) : (
                    ...
                    <div className="w-full mt-4 pt-6 border-t border-white/5 flex items-center justify-between">
                         <div className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-500 group-hover:text-indigo-500 transition-colors">
                            <Radio size={14} /> {channel.group || 'Live'}
                         </div>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-indigo-600/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
      ...
      {filteredChannels.length === 0 && (
          <div className="py-32 text-center">
              ...
              <p className="text-2xl font-bold text-slate-500">No channels found for "{searchQuery}"</p>
          </div>
      )}
    </div>
  );
